...
st.dataframe(
    score_df.style.format({"Prob segna ≥1": lambda x: _fmt_pct_safe(x, 0)}),
    use_container_width=True,
    hide_index=True
)
...
st.dataframe(top3_df.style.format({"Prob": _fmt_pct_safe}), use_container_width=True, hide_index=True)
    ...
    if home_min_score is None:
        ...
        st.dataframe(
            home_df.style.format({
                "Score (calibrato)": lambda x: _fmt_pct_safe(x, 0),
            }),
            use_container_width=True,
            hide_index=True
        )
